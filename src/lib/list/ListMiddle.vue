<template>
  <div>
    ...
    <el-row class="ele-list-middle">
      <template
        v-if="
          filterOptions.direction === 'col' ||
          filterOptions.direction === undefined
        "
      >
  ...
  methods: {
    getFilterOptions() {
      ...
      const setButtonOptions = {
        add: {
          type: '',
          name: $hxt_ele_list_page({ key: 'b51a95-NewAddition', desc: '新增' }),
          fn: () => this.add(), // 新增
        },
        // 失效
        invalid: {
          type: '',
          name: $hxt_ele_list_page({ key: 'b51a95-Invalid', desc: '失效' }),
          fn: () => this.invalid(),
        },
        // 生效
        valid: {
          type: '',
          name: $hxt_ele_list_page({ key: 'b51a95-TakeEffect', desc: '生效' }),
          fn: () => this.valid(),
        },
        //  导出
        export: {
          type: '',
          name: $hxt_ele_list_page({ key: 'b51a95-Export', desc: '导出' }),
          fn: () => this.export(),
        },
        search: {
          type: 'primary',
          name: $hxt_ele_list_page({ key: 'b51a95-Search', desc: '搜索' }),
          fn: () => this.search(),
        },
        clear: {
          type: '',
          name: $hxt_ele_list_page({ key: 'b51a95-Empty', desc: '清空' }),
          fn: () => this.clear(),
        },
        customColumns: {
          type: 'primary',
          name: $hxt_ele_list_page({
            key: 'b51a95-CustomColumnSettings',
            desc: '自定义列设置',
          }),
      ...
      const transferButtonList = (list) => {
        ...
        list.forEach((item) => {
          if (item.filterType) {
            ...
            const disabled =
              item.filterType === 'valid' ||
              item.filterType === 'invalid' ||
              item.isNoCheck
                ? item.disabled ||
            ...
            newObjectList.push({
              ...
              className:
                (item.className || '') +
                ' ' +
                (setButtonOptions[item.filterType]?.className || ''),
              fn:
                item.fn ||
                function () {
                  setButtonOptions[item.filterType] &&
                    setButtonOptions[item.filterType].fn &&
                    setButtonOptions[item.filterType].fn()
                },
              disabled,
            })
